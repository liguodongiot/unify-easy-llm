...
gpu_num=1
echo "执行训练任务脚本："
echo "cd $PROJECT_PATH && ASCEND_RT_VISIBLE_DEVICES=0 torchrun --nproc_per_node=$gpu_num train_unify.py --train_args_file $TRAIN_ARGS_PATH"

cd $PROJECT_PATH && ASCEND_RT_VISIBLE_DEVICES=0 torchrun --nproc_per_node=$gpu_num train_unify.py --train_args_file $TRAIN_ARGS_PATH

# cd $PROJECT_PATH && ASCEND_RT_VISIBLE_DEVICES=0 && python train_unify.py --train_args_file $TRAIN_ARGS_PATH
